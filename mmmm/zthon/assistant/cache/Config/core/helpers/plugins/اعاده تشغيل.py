...
async def _(event):
    ...
    sandy = await edit_or_reply(
        ...
        f"**•⎆┊يتـم الان اعـادة تشغيـل بـوت 𝐒𝐀𝐅𝐄𝐓 قـد يستغـرق الامـر 1-2 دقيقـه ▬▭ ...**",
    )
    try:
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "لـ إطفـاء الداينـو الخاص بتنصيبك بهيروكـو .. لا تستطيع اعاده التشغيل مرة اخرى عبر حسابك عليك الذهاب لحساب هيروكو واتباع الشرح التالي https://t.me/FFP8F/20",
...
async def _(event):
    "لـ إطفـاء البـوت"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "#ايقــاف البــوت\n\n" "**- بـوت 𝐒𝐀𝐅𝐄𝐓 فـي وضــع الايقــاف**")
    await edit_or_reply(event, "**✾╎جــارِ إيقـاف تشغيـل بـوت 𝐒𝐀𝐅𝐄𝐓 الآن 📟 ...**\n\n**✾╎شغِّـلنـي يـدويًـا لاحقًــا**\n**✾╎باتبـاع الشـرح** https://t.me/FFP8F/20")
