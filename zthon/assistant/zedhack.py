...
'''
mm = '''
**- عليك الانضمـام في قنـاة السـورس اولاً**  @FFP8F
'''
...
keyboard = [
  ...
  [
    Button.url("𝗦𝗔𝗙𝗘𝗧™ 𓅛", "https://t.me/FFP8F")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس سيـفت ❤️** \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس سيـفت ❤️**", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond(f"**- ارسل هذا الخطأ الى @SS22TT \n- الخطـأ** str(e)\n")
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("قنـاة السـورس", "https://t.me/FFP8F")
        ]
    ]
